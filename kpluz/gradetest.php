...
if ($subject !== '' && $lesson !== '') {
    ...
    if ($check_result->num_rows > 0) {
        ...
        $update_stmt = $conn->prepare("UPDATE test_results SET score = ?, total_questions = ?, percentage = ? WHERE id = ?");
        $update_stmt->bind_param("iiii", $score, $total, $percentage, $existing_id);
        ...
        $insert_stmt = $conn->prepare("INSERT INTO test_results (user_id, subject, lesson, score, total_questions, percentage) VALUES (?, ?, ?, ?, ?, ?)");
        $insert_stmt->bind_param("issiii", $user_id, $subject, $lesson, $score, $total, $percentage);
